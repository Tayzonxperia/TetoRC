//////// TetoRC Stage 1 Birdbrain shutdown file
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <dirent.h>
...
{
    //execute("/kasane/tetorc/tbin/shutdown");
    clear_directory("/tmp");
    seed_rng_device();
    sync();
...
static void seed_rng_device() 
{
    ...
    if (syscall(SYS_getrandom, seed, 512, 0) < 0) {
        free(seed); // well fuck me sideways
        return;
    }

    int fd = open("/kasane/tetorc/misc/random.seed", O_WRONLY | O_CREAT, 0600);
